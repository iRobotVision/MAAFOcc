...
@HEADS.register_module()
class HeightAwareOccupancyHead(nn.Module):
    """
    基于论文InstanceBEV的Height-Aware设计
    核心改进：
    1. 引入残差学习机制（基础BEV特征 + 高度感知残差）
    2. 使用3D卷积网络增强高度方向特征
    3. 保持原有接口兼容性
    """
    def __init__(self,
        ...
        self.height_res_net = nn.Sequential(
            nn.Conv3d(out_dim, 64, kernel_size=(1, 3, 3), padding=(0, 1, 1)),
            nn.BatchNorm3d(64),
            nn.ReLU(inplace=True),
            nn.Conv3d(64, out_dim, kernel_size=(1, 3, 3), padding=(0, 1, 1)),
        ...
        self.loss_occ = build_loss(loss_occ)
        self.coordinate_transform = build_custom(coordinate_transform) if coordinate_transform else None
    ...
    def forward(self, img_feats, lidar_aug_matrix=None, lidar2ego=None, occ_aug_matrix=None):
        ...
        base_feat = self.base_conv(img_feats)
        base_3d = base_feat.unsqueeze(2)
        height_res = self.height_res_net(base_3d)
        height_res = height_res.squeeze(2)
        fused_feat = base_feat + height_res
        occ_pred = fused_feat.permute(0, 3, 2, 1)  # (B, W, H, C)
        bs, Dx, Dy = occ_pred.shape[:3]
